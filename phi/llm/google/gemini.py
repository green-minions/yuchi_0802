...
class Gemini(LLM):
    ...
    def conform_function_to_gemini(self, params: Dict[str, Any]) -> Dict[str, Any]:
        fixed_parameters = {}
        print("Original params:", json.dumps(params, indent=2))
        ...
        print("Fixed params:", json.dumps(fixed_parameters, indent=2))
    ...
    def add_tool(self, tool: Union[Tool, Toolkit, Callable, Dict, Function]) -> None:
        ...
        self.tool_counter = getattr(self, 'tool_counter', 0) + 1
        print(f"--- Tool #{self.tool_counter} added ---")
        print(f"Tool type: {type(tool).__name__}")
        print("Function declarations:", json.dumps(self.function_declarations, indent=2))

        if self.function_declarations:
            for idx, func in enumerate(self.function_declarations):
                if not func.parameters.get('properties'):
                    print(f"Warning: Function declaration {idx} ({func.name}) has empty properties")
    ...
    def invoke(self, messages: List[Message]):
        print("Invoking Gemini API with the following function declarations:")
        print(json.dumps(self.function_declarations, indent=2))
        print("Full request body:")
        print(json.dumps(self.conform_messages_to_gemini(messages), indent=2))
    ...
    #     )
    def invoke_stream(self, messages: List[Message]):
        converted_messages = self.conform_messages_to_gemini(messages)
        print("Messages sent to Gemini API:")
